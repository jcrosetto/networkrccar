...
public class Cockpit implements KeyListener, ActionListener {
	...
	private JTextField portField = new JTextField(Integer.toString(port), 4); // refers
																				// to
																				// port
																				// server
	// is listening on
	...
	/**
	 *** Constructor *** set up jframe then open socket connection
	 * 
	 * @param host
	 * @param port
	 */
	...
	private void addComponentsToPane(Container pane) {
		...
		indicatorPane.setBorder(BorderFactory
	...
	private boolean openSocket() {
		...
		try {
			if (!axPanel.connected) {
				System.out.println("Re-Connect, connected: "
						+ axPanel.connected);
				axPanel = new AxisCamera();
				// Thread.currentThread().sleep(9000);
				// axPanel.run();
				System.out.println("Re-Connect2, connected: "
						+ axPanel.connected);
	...
	public void actionPerformed(ActionEvent e) {
		if ("toggleConnect".equals(e.getActionCommand())) {
			if (isConnected) {
				...
				speed = 0;
				turnKeyPressed = 0;
				sendOut(); // send out a stopped state before disconnecting
				closeSocket();
				speedGauge.setIcon(speeds[0]);
				leftInd.setIcon(directions[0]);
				rightInd.setIcon(directions[2]);
